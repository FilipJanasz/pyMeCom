...
LAST_CONFIG_PATH = Path('.last_tec_live_logger_gui_config')
MAX_POINTS = 600
MIN_HZ = 0.1


class TecLiveLoggerGui:
    """Standalone TEC-only live logger GUI for legacy Meerstetter COM workflows."""

    def __init__(self, root: Tk):
        self.root = root
        self.root.title('TEC Live Logger GUI')
    ...
    def _build_ui(self) -> None:
        ...
        def add_row(label: str, var, row: int) -> None:
        ...
        Label(conn_frame, text='TEC Connection & Runtime', font=('TkDefaultFont', 10, 'bold')).grid(row=0, column=0, columnspan=4, sticky='w')
        ...
        Label(io_frame, text='TEC Config & Output', font=('TkDefaultFont', 10, 'bold')).grid(row=0, column=0, columnspan=4, sticky='w')
        Label(io_frame, text='Config JSON').grid(row=1, column=0, sticky='w')
        Entry(io_frame, textvariable=self.config_path, width=44).grid(row=1, column=1, columnspan=3, sticky='we')
        ...
        Entry(io_frame, textvariable=self.output_directory, width=42).grid(row=3, column=1, columnspan=3, sticky='we')
        Label(io_frame, text='Output Prefix').grid(row=4, column=0, sticky='w')
        Entry(io_frame, textvariable=self.output_prefix, width=42).grid(row=4, column=1, columnspan=3, sticky='we')
        Label(io_frame, text='This GUI is TEC-only. Use power_live_log_gui.py for the TEC + Huber unified GUI.').grid(row=5, column=0, columnspan=4, sticky='w')
        ...
        Button(buttons, text='Start TEC Logging', command=self.start_logging).pack(side=LEFT, padx=(0, 6))
        ...
        self.request_plot_frame.grid(row=6, column=0, columnspan=4, sticky='nsew', pady=(8, 0))
        self.canvas = None
        self.figure = None
        ...
        if Figure is not None:
            ...
            axis.set_title('Live TEC plot (select columns then start)')
            ...
            self.request_axis.set_title('Requested TEC input from loaded JSON')
            self.request_axis.set_xlabel('Seconds')
            self.request_axis.set_ylabel('TEC voltage / power')
            self.request_axis.grid(True, which='major', linestyle='--', alpha=0.6)
            self.canvas.draw_idle()
            self.request_canvas.draw_idle()
        Checkbutton(io_frame, text='Show requested input line', variable=self.show_requested_line, command=self._redraw_requested_input_plot).grid(row=7, column=0, columnspan=2, sticky='w')
    ...
    def _load_requested_input_from_config(self, path_text: str) -> None:
        ...
        try:
            ...
            for step in schedule:
                ...
                requested = step.get('set_voltage', step.get('power', 0.0))
                requested_value = float(requested or 0.0)
                self.loaded_schedule_points.append((t, requested_value))
                t += duration
                self.loaded_schedule_points.append((t, requested_value))
            total_duration = t
        except Exception:
    ...
    def _apply_double_clicked_column(self, event) -> None:
        ...
        self.selected_cols = [self.columns_list.get(idx)]
    ...
    def start_logging(self) -> None:
        ...
        self._set_controller_status('yellow', 'Controller status: connecting (starting TEC logger)')
        ...
        def worker() -> None:
            try:
                ...
                self.root.after(0, lambda msg=error_message: messagebox.showerror('TEC run failed', msg))
            finally:
                self.animating = False
    ...
    def _redraw_plot(self) -> None:
        ...
        self._draw_series_on_axis(primary_axis, self.selected_cols, 'Live TEC plot')
        if has_second:
            secondary_axis = self.figure.add_subplot(122)
            self._draw_series_on_axis(secondary_axis, self.second_plot_cols, 'Live TEC plot 2')
    ...
    def _draw_series_on_axis(self, axis, columns: list[str], title: str) -> None:
        ...
        for col in columns:
            ...
            if x and y:
                ...
                if self.show_live_line.get():
                    ...
                    marker_style['linewidth'] = 1.0
                else:
                    marker_style['linestyle'] = 'None'
                axis.plot(timestamps, y, label=col, **marker_style)
                plotted_lines += 1
        ...
        if plotted_lines:
            ...
            axis.text(0.5, 0.5, 'Waiting for TEC data', transform=axis.transAxes, ha='center', va='center')
        if mdates is not None:
            axis.xaxis.set_major_formatter(mdates.DateFormatter('%H:%M:%S', tz=timezone.utc))

    def _redraw_requested_input_plot(self) -> None:
        if self.request_figure is None or self.request_canvas is None or self.request_axis is None:
            return
        axis = self.request_axis
        axis.clear()
        axis.set_title('Requested TEC input from loaded JSON')
        axis.set_xlabel('Seconds')
        axis.set_ylabel('TEC voltage / power')
        if self.show_requested_line.get() and self.loaded_schedule_points:
            x_vals = [x for x, _ in self.loaded_schedule_points]
            y_vals = [y for _, y in self.loaded_schedule_points]
            axis.plot(x_vals, y_vals, color='tab:orange', linewidth=1.2, marker='o', markersize=2.5, label='requested TEC')
            axis.legend(loc='best')
        elif self.show_requested_line.get():
            axis.text(0.5, 0.5, 'No TEC power_schedule loaded', transform=axis.transAxes, ha='center', va='center')
        else:
            axis.text(0.5, 0.5, 'Requested input hidden (enabled by checkbox)', transform=axis.transAxes, ha='center', va='center')
        axis.grid(True, which='major', linestyle='--', alpha=0.6)
