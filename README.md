...
JSON loading/template generation and recipe editing are split into separate tabs for compactness: **Example Loader** contains the config path, output settings, template-curve fields, and loaded JSON preview; **Example Editor** contains the recipe builder. The **Recipe Builder** supports GUI-created shared JSON files and now keeps the step-entry fields in a compact left panel with the editable recipe table beside it, leaving more vertical room for the preview plot. Operators can add or update table rows with duration, Huber temperature, TEC voltage/current, and/or TEC preview power, or use **Load Existing JSON** in the editor to pull an existing unified or legacy TEC example from disk into the editable table before saving a variant. The preview plot shows step curves for TEC requested power and Huber bath temperature; clicking in a preview subplot copies a point into the editable fields. **Save Recipe JSON** writes the same `steps[]` + `safety` schema used by loaded unified configs.
