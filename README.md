...
The unified GUI keeps device setup in one top-left **Connection Detection** pane and keeps non-port run settings in a separate **Runtime Options** pane below it. The connection pane has side-by-side TEC and Huber sections with their own port fields, detect buttons, colored indicators, and status text. TEC-specific connection settings such as port, serial hint, autodetect, and address live with TEC detection; the Huber port lives with Huber detection. Runtime-only settings such as run mode, channel, acquisition rate, duration, bath standby temperature, and pump safe state are no longer mixed into the connection controls. The run-mode selector now has **Huber-only** next to Auto/TEC-only/Unified for bath-only recipes.

A **Manual Commands** pane provides basic operator commands without changing JSON schedules: set TEC power, set TEC voltage/current, zero TEC output, set Huber setpoint, apply Huber pump on/off, and read Huber bath/setpoint values. These controls still call the workflow adapters rather than mixing hardware protocol logic into the GUI.

The **Recipe Builder** in the Config & Output area supports GUI-created shared JSON files. Operators can add or update table rows with duration, Huber temperature, TEC voltage/current, and/or TEC power. The preview plot shows BIOS-style step curves for TEC requested power and Huber bath temperature; clicking in a preview subplot copies a point into the editable fields. **Save Recipe JSON** writes the same `steps[]` + `safety` schema used by loaded unified configs.

Use **Scan COM Ports** when automatic detection does not choose the expected device. The scan result now shows one COM device per line in the popup and a short, wrapping summary in the pane, with guidance to copy a COM name such as `COM3` into the TEC or Huber port field before clicking **Detect TEC** or **Detect Huber**. TEC detection now matches the standalone TEC-only GUI by treating a successful MeCom serial open as a detected TEC, then attempts the controller address query only as extra verification; if the address query fails, the port is still kept in the port field and TEC serial autodetect is disabled for subsequent starts to avoid fragile autodetect ordering. Huber detection now loads the bundled `huberStuff/pyPbCmd/HuberPkg/huber_thermostat` client when no separately installed `huber_thermostat` package is found, including when the legacy Huber app is launched from inside `huberStuff/pyPbCmd`, so it can use the legacy 9600-baud `TI?` ping scan instead of falling back to simulation simply because the package was not installed. Long TEC/Huber detection messages are clipped and wrapped so connection details do not force the GUI wider.

Huber connection scan fix plan:
1. Keep the Huber low-level client separate from TEC code and call it only through `huberStuff/pyPbCmd/huber_adapter.py` / `workflows/automation/huber/adapter.py`.
2. Ensure the adapter can load either an installed `huber_thermostat` package or the bundled legacy package under `huberStuff/pyPbCmd/HuberPkg`.
3. Use the legacy detection sequence: enumerate `serial.tools.list_ports.comports()`, open each candidate at 9600 baud with a 1 s timeout, send `TI?\r\n`, and accept a response beginning with `TI`.
