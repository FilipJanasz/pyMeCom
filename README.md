...
JSON template and recipe editing now live on their own **JSON Example Editor** tab. The **Recipe Builder** supports GUI-created shared JSON files. Operators can add or update table rows with duration, Huber temperature, TEC voltage/current, and/or TEC preview power. The preview plot shows step curves for TEC requested power and Huber bath temperature; clicking in a preview subplot copies a point into the editable fields. **Save Recipe JSON** writes the same `steps[]` + `safety` schema used by loaded unified configs.

Use the single **Scan COM Ports** button when automatic detection does not choose the expected device. The scan result shows one COM device per line directly in the Connection Detection pane, with guidance to copy a COM name such as `COM3` into the TEC or Huber port field before clicking **Detect TEC** or **Detect Huber**; it no longer opens a duplicate popup window. TEC detection now matches the standalone TEC-only GUI by treating a successful MeCom serial open as a detected TEC, then attempts the controller address query only as extra verification; if the address query fails, the port is still kept in the port field and TEC serial autodetect is disabled for subsequent starts to avoid fragile autodetect ordering. Huber detection now loads the bundled `huberStuff/pyPbCmd/HuberPkg/huber_thermostat` client when no separately installed `huber_thermostat` package is found, including when the legacy Huber app is launched from inside `huberStuff/pyPbCmd`, so it can use the legacy 9600-baud `TI?` ping scan instead of falling back to simulation simply because the package was not installed. Long TEC/Huber detection messages are clipped and wrapped so connection details do not force the GUI wider.
...
- In the **JSON Example Editor** tab, the GUI shows editable fields for:
