...
Use **Scan COM Ports** when automatic detection does not choose the expected device. The scan result now shows one COM device per line in the popup and a short, wrapping summary in the pane, with guidance to copy a COM name such as `COM3` into the TEC or Huber port field before clicking **Detect TEC** or **Detect Huber**. TEC detection performs a controller address query, then keeps the detected port in the port field and disables TEC serial autodetect for subsequent starts to avoid fragile autodetect ordering. Huber detection now loads the bundled `huberStuff/pyPbCmd/HuberPkg/huber_thermostat` client when no separately installed `huber_thermostat` package is found, including when the legacy Huber app is launched from inside `huberStuff/pyPbCmd`, so it can use the legacy 9600-baud `TI?` ping scan instead of falling back to simulation simply because the package was not installed. Long TEC/Huber detection messages are clipped and wrapped so connection details do not force the GUI wider.
